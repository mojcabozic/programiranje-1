...
let rec insert x k = function
  |y :: ys -> if k <= 0 then x :: y :: ys else insert x (k-1) ys
  |[] -> [x]
...
let rec largest = function
  | x :: y :: ys -> if x >= y then (largest (x :: ys)) else (largest (y :: ys))
  | x :: [] -> x

let rec second_largest list = 
  match list with
  | x :: [] -> x
  | x :: xs -> if x = largest list && x != largest xs then largest xs else second_largest xs
